...
#define TEST_IOPB_ADDR 0x2200
#define TEST_HDCINT 0x04
#define TEST_RBYT_NOT_READY 0x80
#define TEST_DISK_SIZE 3796992
#define TEST_ARRAY_SIZE(array) (sizeof(array) / sizeof((array)[0]))

extern UNIT isbc206_unit[];
...
/*
 * Provide the fake multibus registration hook used by the iSBC 206 setup
...
/*
 * Read one byte from the fake system memory image used for iSBC 206 IOPBs.
 */
uint8 get_mbyte(uint16 addr)
{
 ...
 */
static int setup_isbc206(void **state)
{
    (void)state;
    ...
    memset(&hdc206, 0, sizeof(hdc206));
    for (uint32 i = 0; i < ISBC206_HDD_NUM; ++i) {
        isbc206_unit[i].flags = 0;
        isbc206_unit[i].filebuf = NULL;
        isbc206_unit[i].u6 = i;
    }
    isbc206_unit[0].flags = UNIT_ATT;
    isbc206_unit[0].filebuf = test_disk;
    isbc206_unit[0].capac = TEST_DISK_SIZE;
    isbc206_reset_dev();
...
/*
 * Start an iSBC 206 operation through the controller's public I/O ports.
 */
 ...
 * documented iSBC 206 I/O ports.
 */
static void test_set_port_accepts_hex_port(void **state)
{
    const uint16 expected_ports[] = {0x68, 0x69, 0x6a, 0x6b, 0x6f};

    (void)state;

    assert_int_equal(isbc206_set_port(&isbc206_unit[0], 0, "68", NULL),
                     SCPE_OK);

    assert_int_equal(hdc206.baseport, 0x68);
...
static void test_set_port_rejects_missing_hex_value(void **state)
{
    ...
    hdc206.baseport = 0x68;
    assert_int_equal(isbc206_set_port(&isbc206_unit[0], 0, "not-hex", NULL),
                     SCPE_ARG);
...
static void test_set_port_rejects_trailing_junk(void **state)
{
    ...
    hdc206.baseport = 0x68;
    assert_int_equal(isbc206_set_port(&isbc206_unit[0], 0, "68junk", NULL),
                     SCPE_ARG);
...
static void test_set_port_rejects_out_of_range_value(void **state)
{
    ...
    hdc206.baseport = 0x68;
    assert_int_equal(isbc206_set_port(&isbc206_unit[0], 0, "100", NULL),
                     SCPE_ARG);
...
static void test_set_port_rejects_null_value(void **state)
{
    ...
    hdc206.baseport = 0x68;
    assert_int_equal(isbc206_set_port(&isbc206_unit[0], 0, NULL, NULL),
                     SCPE_ARG);
...
static void test_set_int_accepts_hex_interrupt(void **state)
{
    ...
    assert_int_equal(isbc206_set_int(&isbc206_unit[0], 0, "05", NULL), SCPE_OK);

    assert_int_equal(hdc206.intnum, 0x05);
}
...
static void test_set_int_rejects_missing_hex_value(void **state)
{
    ...
    hdc206.intnum = 0x05;
    assert_int_equal(isbc206_set_int(&isbc206_unit[0], 0, "not-hex", NULL),
                     SCPE_ARG);
...
static void test_set_int_rejects_trailing_junk(void **state)
{
    ...
    hdc206.intnum = 0x05;
    assert_int_equal(isbc206_set_int(&isbc206_unit[0], 0, "05junk", NULL),
                     SCPE_ARG);
...
static void test_set_int_rejects_out_of_range_value(void **state)
{
    ...
    hdc206.intnum = 0x05;
    assert_int_equal(isbc206_set_int(&isbc206_unit[0], 0, "100", NULL),
                     SCPE_ARG);
...
static void test_set_int_rejects_null_value(void **state)
{
    ...
    hdc206.intnum = 0x05;
    assert_int_equal(isbc206_set_int(&isbc206_unit[0], 0, NULL, NULL),
                     SCPE_ARG);
 ...
 * so a disabled interrupt should not raise HDCINT for a not-ready drive.
 */
static void test_not_ready_completion_honors_interrupt_disable(void **state)
{
    (void)state;

    isbc206_unit[0].flags = 0;
    isbc206_reset_dev();
    write_nop_iopb(0x10);
    start_iopb();
...
int main(void)
{
    const struct CMUnitTest tests[] = {
        cmocka_unit_test_setup(test_set_port_accepts_hex_port, setup_isbc206),
        ...
        cmocka_unit_test_setup(test_set_port_rejects_out_of_range_value,
                               setup_isbc206),
        cmocka_unit_test_setup(test_set_port_rejects_null_value, setup_isbc206),
        ...
        cmocka_unit_test_setup(test_set_int_rejects_out_of_range_value,
                               setup_isbc206),
        cmocka_unit_test_setup(test_set_int_rejects_null_value, setup_isbc206),
        cmocka_unit_test_setup(
            test_nop_iopb_default_channel_word_sets_interrupt, setup_isbc206),
        cmocka_unit_test_setup(
            test_nop_iopb_interrupt_disable_suppresses_interrupt,
            setup_isbc206),
        cmocka_unit_test_setup(
            test_not_ready_completion_honors_interrupt_disable, setup_isbc206),
    };
