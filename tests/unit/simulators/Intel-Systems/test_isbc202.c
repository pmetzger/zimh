...
#define TEST_IOPB_ADDR 0x2000
#define TEST_FDCINT 0x04
#define TEST_RBYT_NOT_READY 0x80
#define TEST_DISK_SIZE 512512
 ...
 */
static int setup_isbc202(void **state)
{
    (void)state;
    ...
    isbc202_unit[0].flags = UNIT_ATT;
    isbc202_unit[0].filebuf = test_disk;
    isbc202_unit[0].capac = TEST_DISK_SIZE;
    isbc202_reset_dev();
...
/*
 * Start an iSBC 202 operation through the controller's public I/O ports.
 */
static void start_iopb(void)
{
    isbc202r1(TRUE, TEST_IOPB_ADDR & 0xff, 0);
    isbc202r2(TRUE, TEST_IOPB_ADDR >> 8, 0);
}

/*
 * A valid port setting should update the controller base and register all
 * documented iSBC 202 I/O ports.
 */
static void test_set_port_accepts_hex_port(void **state)
{
    ...
    assert_int_equal(isbc202_set_port(&isbc202_unit[0], 0, "78", NULL),
                     SCPE_OK);

    assert_int_equal(fdc202.baseport, 0x78);
...
static void test_set_port_rejects_missing_hex_value(void **state)
{
    ...
    fdc202.baseport = 0x78;
    assert_int_equal(isbc202_set_port(&isbc202_unit[0], 0, "not-hex", NULL),
                     SCPE_ARG);
...
static void test_set_port_rejects_trailing_junk(void **state)
{
    ...
    fdc202.baseport = 0x78;
    assert_int_equal(isbc202_set_port(&isbc202_unit[0], 0, "78junk", NULL),
                     SCPE_ARG);
...
static void test_set_port_rejects_out_of_range_value(void **state)
{
    ...
    fdc202.baseport = 0x78;
    assert_int_equal(isbc202_set_port(&isbc202_unit[0], 0, "100", NULL),
                     SCPE_ARG);
...
static void test_set_port_rejects_null_value(void **state)
{
    ...
    fdc202.baseport = 0x78;
    assert_int_equal(isbc202_set_port(&isbc202_unit[0], 0, NULL, NULL),
                     SCPE_ARG);
...
static void test_set_int_accepts_hex_interrupt(void **state)
{
    ...
    assert_int_equal(isbc202_set_int(&isbc202_unit[0], 0, "05", NULL), SCPE_OK);

    assert_int_equal(fdc202.intnum, 0x05);
}
...
static void test_set_int_rejects_missing_hex_value(void **state)
{
    ...
    fdc202.intnum = 0x05;
    assert_int_equal(isbc202_set_int(&isbc202_unit[0], 0, "not-hex", NULL),
                     SCPE_ARG);
...
static void test_set_int_rejects_trailing_junk(void **state)
{
    ...
    fdc202.intnum = 0x05;
    assert_int_equal(isbc202_set_int(&isbc202_unit[0], 0, "05junk", NULL),
                     SCPE_ARG);
...
static void test_set_int_rejects_out_of_range_value(void **state)
{
    ...
    fdc202.intnum = 0x05;
    assert_int_equal(isbc202_set_int(&isbc202_unit[0], 0, "100", NULL),
                     SCPE_ARG);
...
static void test_set_int_rejects_null_value(void **state)
{
    ...
    fdc202.intnum = 0x05;
    assert_int_equal(isbc202_set_int(&isbc202_unit[0], 0, NULL, NULL),
                     SCPE_ARG);
...
static void test_nop_iopb_default_channel_word_sets_interrupt(void **state)
{
    ...
    assert_true(isbc202r0(FALSE, 0, 0) & TEST_FDCINT);
    assert_int_equal(isbc202r3(FALSE, 0, 0), 0);
}
...
static void test_nop_iopb_interrupt_disable_suppresses_interrupt(void **state)
{
    ...
    assert_false(isbc202r0(FALSE, 0, 0) & TEST_FDCINT);
    assert_int_equal(isbc202r3(FALSE, 0, 0), 0);
}
...
static void test_not_ready_completion_honors_interrupt_disable(void **state)
{
    ...
    isbc202_unit[0].flags = 0;
    isbc202_reset_dev();
    write_nop_iopb(0x10);
    start_iopb();

    assert_false(isbc202r0(FALSE, 0, 0) & TEST_FDCINT);
    assert_int_equal(isbc202r3(FALSE, 0, 0), TEST_RBYT_NOT_READY);
}

int main(void)
{
    const struct CMUnitTest tests[] = {
        cmocka_unit_test_setup(test_set_port_accepts_hex_port, setup_isbc202),
        ...
        cmocka_unit_test_setup(test_set_port_rejects_out_of_range_value,
                               setup_isbc202),
        cmocka_unit_test_setup(test_set_port_rejects_null_value, setup_isbc202),
        ...
        cmocka_unit_test_setup(test_set_int_rejects_out_of_range_value,
                               setup_isbc202),
        cmocka_unit_test_setup(test_set_int_rejects_null_value, setup_isbc202),
        cmocka_unit_test_setup(
            test_nop_iopb_default_channel_word_sets_interrupt, setup_isbc202),
        cmocka_unit_test_setup(
            test_nop_iopb_interrupt_disable_suppresses_interrupt,
            setup_isbc202),
        cmocka_unit_test_setup(
            test_not_ready_completion_honors_interrupt_disable, setup_isbc202),
    };
