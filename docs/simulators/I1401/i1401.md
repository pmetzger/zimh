...
[2 IBM 1401 Features](#ibm-1401-features)
[2.1 CPU](#cpu)
[2.2 1402 Card Reader/Punch (CDR, CDP, STKR)](#card-readerpunch-cdr-cdp-stkr)
[2.3 1403 Line Printer (LPT)](#line-printer-lpt)
[2.4 1407 Inquiry Terminal (INQ)](#inquiry-terminal-inq)
[2.5 1311 Disk Pack (DP)](#disk-pack-dp)
[2.6 729 Magnetic Tape (MT)](#magnetic-tape-mt)
[3 Symbolic Display and Input](#symbolic-display-and-input)
[4 Character Sets](#character-sets)
[4.1 Old Conversions](#old-conversions)
...
The card reader supports the `BOOT` command. `BOOT CDR` reads a card
image into locations 1-80, sets a word mark under location 1, clears
storage, and then transfers control to location 1.
...
| stacker | OS I/O error | print error message; if `IOCHK` set, report error and stop |
...
This loads a carriage control tape file.
...
request character is typed in. The latter cancels the type-in and sets
`INC`.
...
is denoted by `` ` `` and must precede the character to be marked. All
other devices can only accept single character input, without word
marks.
...
| BCD code | ASCII representation | IBM 1401 print character chains |
|---|---|---|
| 00 | `space` |  |
| 01 | `1` |  |
| 02 | `2` |  |
...
| Command | Action |
|---|---|
| `SET CPU OLDCONVERSIONS` | use the old conversion conventions |
| `SET CPU NEWCONVERSIONS` | use the new conversion conventions |
