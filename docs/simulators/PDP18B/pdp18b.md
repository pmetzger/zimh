...
[2. 18b PDP Features](#18b-pdp-features)

[2.1 CPU](#cpu)

[2.2 Floating Point Processor (FPP)](#floating-point-processor-fpp)

[2.3 Programmed I/O Devices](#programmed-io-devices)

[2.3.1 Paper Tape Reader (PTR)](#paper-tape-reader-ptr)

[2.3.2 Paper Tape Punch (PTP)](#paper-tape-punch-ptp)

[2.3.3 Terminal Input (TTI)](#terminal-input-tti)

[2.3.4 Terminal Output (TTO)](#terminal-output-tto)

[2.3.5 Line Printers (LPT, LP9)](#line-printers-lpt-lp9)

[2.3.6 Real-Time Clock (CLK)](#real-time-clock-clk)

[2.3.7 Additional Terminals (TTIX, TTOX)](#additional-terminals-ttix-ttox)

[2.4 RP15/RP02/RP03 Disk Pack (RP)](#rp15rp02rp03-disk-pack-rp)

[2.5 Type 24/RM09 Serial Drum (DRM)](#type-24rm09-serial-drum-drm)

[2.6 RB09 Fixed Head Disk (RB)](#rb09-fixed-head-disk-rb)

[2.7 RF09/RF15/RS09 Fixed Head Disk (RF)](#rf09rf15rs09-fixed-head-disk-rf)

[2.8 Type 550/555, TC02/TU55, and TC15/TU56 DECtape (DT)](#type-550555-tc02tu55-and-tc15tu56-dectape-dt)

[2.9 TC59/TU10 Magnetic Tape (MT)](#tc59tu10-magnetic-tape-mt)

[2.10 DR15C Parallel Interface (PDP-15/76 only)](#dr15c-parallel-interface-pdp-1576-only)

[3. Symbolic Display and Input](#symbolic-display-and-input)

[4. Character Sets](#character-sets)
...
appropriate switch in the compilation command line:
...
The four 18b PDPs (PDP-4, PDP-7, PDP-9, PDP-15) are very similar and
...
- An unimplemented instruction is decoded, and register `STOP_INST` is
  set.
- More than `XCT_MAX` nested executes are detected during instruction
  execution.
- An `FP15` instruction is decoded, the `FP15` is disabled, and
  register `STOP_FPP` is set.
...
(hardware versus second stage is determined from the data); files
ending in any other extension are assumed to be binary loader format.
Examples:
...
If no address is given for a `RIM` format load, a starting address of
200 (octal) is assumed.

The `DUMP` command is not supported.

## CPU

The CPU options are the presence of the `EAE`, the presence of the
`API` and memory protection (for the PDP-9 and PDP-15), the presence
of relocation or `XVM` (PDP-15 only), and the size of main memory.
...
Memory sizes greater than 8K are only available on the PDP-7, PDP-9,
and PDP-15; memory sizes greater than 32KW are only available on the
PDP-15. If memory size is being reduced, and the memory being
truncated contains non-zero data, the simulator asks for confirmation.
Data in the truncated portion of memory is lost. Initial memory size
is 8K for the PDP-4, 32K for the PDP-7 and PDP-9, and 128K for the
PDP-15.

The `PROT` option corresponds to the `KX09A` on the PDP-9 and the
`KM15` for the PDP-15. The `PROT` option is required to run the
...
interrupts disabled (`DECSYS`) or a `JMP *` loop (`XVM/RSX`). There is
no idle loop detector for `ADSS`, `F/B`, or `DOS`.
...
(`FPP`). The `FPP` can be enabled and disabled; by default it is
disabled.
...
and `-K` for KSR mode. In ASCII mode, data returned by the read
alphabetic command has even parity. This allows normal text files to
be used as input to the paper tape reader on the PDP-9 and PDP-15. In
KSR mode, data returned by the read alphabetic command has forced ones
parity. This allows normal text files to be used as input to the paper
tape reader on the PDP-7.
...
existing file. A new file can be created if you attach with the `-N`
switch.

The `ATTACH PTP` command recognizes one switch, `-A` for ASCII mode.
In ASCII mode, data is punched with the high-order bit clear, and
`NULL` and `DEL` characters are suppressed. This allows punch output
to be processed with normal text editing utilities.
...
On the PDP-7, PDP-9, and PDP-15, the terminal interfaces (`TTI`,
`TTO`) can be set to one of four modes, `KSR`, `7P`, `7B`, or `8B`.
On the PDP-7 and PDP-9, `Unix v0` mode is also available:
...
the `LP09` line printer controller for the PDP-9. It may be needed on
the PDP-15 to run certain software packages. `LP9` is disabled by
default.

The `LP15` is a 3-cycle data break device. The current address
register is in memory. It can be examined and modified with `SET` and
`SHOW` commands:
...
The additional terminals consist of two independent devices, `TTIX`
and `TTOX`. The entire set is modeled as a terminal multiplexer, with
`TTIX` as the master unit. The additional terminals perform input and
output through Telnet sessions connected to a user-specified port. The
...
additional terminals. The number of additional terminals can be
...
The additional terminals can be set to one of four modes, `KSR`, `7P`,
`7B`, or `8B`:
...
open until disconnected either by the Telnet client, a
`SET TTOXn DISCONNECT` command, or a `DETACH TTIX` command.
...
`RP15` options include the ability to make units write enabled or
write locked and to select the type of disk drive:
...
Drum data files are buffered in memory; therefore, end-of-file and
OS I/O errors cannot occur.

## RB09 Fixed Head Disk (RB)

The `RB09` was an early fixed-head disk for the PDP-7 and PDP-9. It
was superseded by the `RF09/RS09`. It is disabled by default.
...
`RB09` data files are buffered in memory; therefore, end-of-file and
OS I/O errors cannot occur.

## RF09/RF15/RS09 Fixed Head Disk (RF)

`RF09/RF15` options include the ability to set the number of platters
to a fixed value between 1 and 8, or to autosize the number of
...
The `RF09/RF15` is a 3-cycle data break device. The word count and
...
The `RF09/RF15` is a three-cycle data break device. If `BURST = 0`,
word transfers are scheduled individually; if `BURST = 1`, the entire
...
`RF15/RF09` data files are buffered in memory; therefore, end-of-file
...
controller. The PDP-9 uses the `TC02`, and the PDP-15 uses the
`TC15`. The `TC02` and `TC15` are DMA controllers and programmatically
identical. Except for the first five units of the Type 550,
PDP-4/7/9/15 DECtape format has 5 18b words in the block header and
trailer, like all other DECtapes.

In the 550/555, DECtape drives are numbered 1-8; in the simulator,
drive 8 is unit 0. In the `TC02/TC15`, DECtape drives are numbered
0-7. DECtape options include the ability to make units write enabled
or write locked.
...
The Type 550, `TC02`, and `TC15` support PDP-8 format, PDP-11 format,
and 18b format DECtape images. `ATTACH` assumes the image is in 18b
format; the user can force other choices with switches:
...
track, and block header and trailer, are not stored. Thus, the
`WRITE TIMING AND MARK TRACK` function is not supported; the
`READ ALL` function always returns the hardware standard block header
...
address registers are in memory. They can be examined and modified
with `SET` and `SHOW` commands:
...
The `DR15C` is a parallel interface that provides the PDP-15 side of
the `UC15` control interface in a PDP-15/76 system. It is disabled by
default. Enabling the `DR` creates the shared memory and status
interfaces for communicating with the `UC15`.
...
The 18b PDP simulators implement symbolic display and input. Display
...
(PDP-15 only), `IOT`, operate, `LAW`, `FP15` memory reference
(PDP-15 only), and `FP15` no operand (PDP-15 only).
...
where `I` (PDP-4, PDP-7) / `*` (PDP-9, PDP-15) signifies indirect
reference, and `X` signifies indexing (PDP-15 in page mode only). The
address is an octal number in the range `0 - 017777` (PDP-4, PDP-7,
PDP-9, and PDP-15 in bank mode) or `0 - 07777` (PDP-15 in page mode).

`IOT` instructions consist of single mnemonics, e.g. `KRB`, `TLS`.
`IOT` instructions may be ORed together:

```text
iot iot iot...
```

`IOT`s may also include the number `10`, signifying clear the
accumulator:
...
`IOT`s for which there is no opcode may be specified as `IOT n`, where
`n` is an octal number in the range `0 - 07777`.
...
`EAE`s for which there is no opcode may be specified as `EAE n`, where
`n` is an octal number in the range `0 - 037777`.
...
`FP15` memory-reference instructions occupy two successive words and
have the format:
...
where `*` signifies indirect addressing. The address is a number in
the range `0 - 0377777`.

`FP15` no-operand instructions occupy two successive words and have
the format:
...
was Baudot. The PDP-4's line printer used a modified Hollerith
character set. The PDP-7's and PDP-9's consoles were `KSR-33`
Teletypes; their character sets were basically ASCII. The PDP-7's and
PDP-9's line printers used `SIXBIT` encoding (ASCII codes `040 - 0137`
masked to six bits). The PDP-15's I/O devices were all ASCII. The
following table provides equivalences between ASCII characters and the
PDP-4's I/O devices. In the console table, `FG` stands for figures
(upper case).
...
DECsys Baudot packs the five-bit character code, and the
figure/letters flag, into six bits as follows:
