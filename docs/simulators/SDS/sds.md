...
until disconnected either by the Telnet client, a `SET MUX DISCONNECT`
command, or a `DETACH MUX` command.
...
| Command | Action |
|---|---|
| `SET DSK LOCKED` | set write locked |
| `SET DSK WRITEENABLED` | set write enabled |
...
| mem reference | `addr{,tag}` | `STA* 300` | indirect addr |
...
| SYSPOP (prog op) | `op,addr{,tag}` | `SBRM 500` |  |
| SYSPOP (prog op) | `op,addr{,tag}` | `CIN* 400,2` |  |
